...
def parse_args():
    ...
    # obs相关参数
    parser.add_argument('--obs_ak', nargs='?', type=str,
                        default=os.environ.get('OBS_AK'), help="OBS AK")
    parser.add_argument('--obs_sk', nargs='?', type=str,
                        default=os.environ.get('OBS_SK'), help="OBS SK")
    ...
    # 解析参数
    parser.add_argument('--learning_rate_scope', type=str,
                        default='[]', help='learning rate scope')
    parser.add_argument('--batch_size_scope', type=str,
                        default='[]', help='batch size scope')
    parser.add_argument('--momentum_scope', type=str,
                        default='[]',  help='momentum scope')
    # 解析参数
...
def runAim(records, repo, learning_rate, momentum, batch_size, kind="LossMonitor"):
    ...
    if kind == "LossMonitor":
        ...
        for x in records:
            # 横坐标为epoch，纵坐标为loss
            aim_run.track(float(x['loss']), name='loss',
                          epoch=x['epoch'], context={"subset": "train"})
            # 横坐标为epoch，纵坐标为一次epoch耗费的时间
            aim_run.track(float(x['time']), name='time',
                          epoch=x['epoch'], context={"subset": "train"})
    ...
    if kind == "ValAccMonitor":
        for x in records:
            ...
            aim_run.track(float(x['loss']), name='loss',
                          epoch=x['epoch'], context={"subset": "train"})
            # 横坐标为epoch，纵坐标为一次epoch耗费的时间
            aim_run.track(float(x['acc']), name='time',
                          epoch=x['epoch'], context={"subset": "test"})
...
def main(args):
    ...
    print("this is matrics!!", learning_rate_scope,
          batch_size_scope, momentum_scope)
    ...
    # 根绝metrics分组，补齐数据
    aimlog_list = groupTraceLog(
        aimlog_list, batch_size_scope, learning_rate_scope, momentum_scope)
